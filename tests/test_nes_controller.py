...
def test_physical_controller_compiles_with_four_passive_limited_axes():
    ...
    for name in expected - {"passive_ab_press"}:
        ...
        assert model.jnt_stiffness[joint_id] == pytest.approx(8.0)
        dof_id = model.jnt_dofadr[joint_id]
        assert model.dof_damping[dof_id] == pytest.approx(0.12)
...
def test_dpad_and_ab_hysteresis_prevent_threshold_flicker():
    controller = NESController()
    assert controller.update(radians(1.5), 0, radians(1.5)).right
    held = controller.update(radians(1.0), 0, radians(1.0))
    assert held.right and held.a
    released = controller.update(radians(0.7), 0, radians(0.7))
    assert not released.right and not released.a


def test_ab_chord_press_has_its_own_hysteresis():
    controller = NESController()
    chord = controller.update(0, 0, 0, 0.0017)
    assert chord.a and chord.b
    held = controller.update(0, 0, 0, 0.0012)
    assert held.a and held.b
    released = controller.update(0, 0, 0, 0.0009)
